...
onMounted(async () => {
  ...
  timer.value = setInterval(() => {
    ...
    timeleft.value = DateTime.fromISO(nextDay)
      .startOf('day')
      .diffNow('seconds')
      .as('seconds')
      .toFixed(0);
  ...
  <div class="tw_mt-6 tw_p-2">
    ...
    <div v-if="allMovies.length !== filmStore.totalMovieCount" class="tw_mb-8 tw_text-center">
      ...
      <h5>
        Next Game in
        <span class="mono">{{
          Duration.fromObject({ seconds: timeleft }).toFormat('hh:mm:ss')
        }}</span>
      </h5>
    </div>
    ...
    >
      <q-btn
        ...
        class="tw_inline-block tw_w-[80px] tw_rounded tw_bg-accent tw_px-4 tw_py-2 tw_text-center"
      >
        # {{ movie.id }}
      </q-btn>
      <div class="tw_flex tw_justify-center">
        <div
          ...
          :class="
            scoreStore.rawScores[movie.id]
              ? i >= scoreStore.rawScores[movie.id]
                ? i == scoreStore.rawScores[movie.id]
                  ? 'tw_bg-positive'
                  : 'tw_bg-[#6b7d95]'
                : 'tw_bg-negative'
              : 'tw_bg-[#6b7d95] tw_opacity-50'
          "
        />
      </div>
      <span class="tw_w-[90px] tw_text-center">
        {{
          scoreStore.rawScores[movie.id]
            ? scoreStore.rawScores[movie.id] <= 5
              ? '🏆 Won'
              : '😞 Lost'
            : ' Not Played'
        }}
